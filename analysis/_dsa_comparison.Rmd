...
All `r comma(length(peaks))` consensus peaks were loaded and the 75^th^ percentile of peaks widths was found to be `r fixed_width`bp.
This was then set as the fixed width for all peaks and any resized peaks which overlapped black- or grey-listed regions were subsequently excluded, give a final set of `r comma(length(fw_peaks))` peaks fixed at `r fixed_width`bp.
...
After performing this step, the quantro statistics obtained from the set of permutations was subset to only the unique values, assuming that each permutation will return a distinct statistic.
...
Alignments were counted for the set of sliding windows using windows of `r window_size`bp shifting in increments of `r window_step`bp.
...
As a result ranges were retained from the top `r percent(metadata(se_sliding)$cuts$prop, accuracy = 0.01)` signal percentiles, wher signal was beyond `r round(metadata(se_sliding)$cuts$control, 2)` times the level of signal in the input samples.
...
tbl <- df %>% 
  ...
  reactable(
    ...
    defaultColDef = colDef(
      ...
      html = TRUE, filterMethod = js_greater,
      style = \(value) {
        col <- rgb(0, 0.6, 0.1, 0)
