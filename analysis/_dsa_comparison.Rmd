...
### Comparison of Identical Counts {.tabset}

In order to confirm there were no algorithmic differences between `DiffBind` and the Wald-Test as implemented in `extraChIPs`, the counts returned by DiffBind for the re-centred peaks were analysed using the `extraChIPs` implementation of the Wald Test.

#### Library Size

```{r compare-db-ec-ls, fig.cap = "*Exact comparison between p-values obtained using the same set of counts when using the DiffBind implementation of the Wald Test and the extraChIPs implementation. Library Size normalisation was used for both analyses.*"}
db_ec_ls_wt <- dba_ls %>% 
  dba(bSummarizedExperiment = TRUE) %>% 
  fitAssayDiff(
    assay = "Reads", 
    design = model.matrix(~Treatment, data = colData(.)),
    lib.size = "Reads", norm = "none", method = "wald", asRanges = TRUE
  )
db_ec_ls_wt %>% 
  select(ec = PValue) %>% 
  join_overlap_inner(db_fw_res$`db-fw-ls-wt`) %>% 
  as_tibble() %>% 
  ggplot(aes(-log10(PValue), -log10(ec))) +
  geom_point() +
  geom_abline(slope = 1) +
  labs(
...
```{r compare-db-ec-rle, fig.cap = "*Exact comparison between p-values obtained using the same set of counts when using the DiffBind implementation of the Wald Test and the extraChIPs implementation. RLE normalisation was used for both analyses.*"}
db_ec_rle_wt <- dba_rle %>% 
  dba(bSummarizedExperiment = TRUE) %>% 
  fitAssayDiff(
    assay = "Reads", 
    design = model.matrix(~Treatment, data = colData(.)),
    lib.size = "Reads", norm = "RLE", method = "wald", asRanges = TRUE
  )
db_ec_rle_wt %>% 
  select(ec = PValue) %>% 
  join_overlap_inner(db_fw_res$`db-fw-rle-wt`) %>% 
  as_tibble() %>% 
  ggplot(aes(-log10(PValue), -log10(ec))) +
  geom_point() +
  geom_abline(slope = 1) +
